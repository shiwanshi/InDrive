        ...
        <View style={{ flex: 0 }}>
            ...
            <View style={{ flexDirection: 'column', alignItems: 'center', padding: 16 }}>
              {prompts.map((prompt, index) => (
                <View key={index} style={{ marginVertical: 8 }}>
                  {prompt.url ? (
                    <Button color='green'title={prompt.text} onPress={() => handlePromptSelection(prompt)} style={styles.button} />
                  ) : (
                    <Button color='green' title={prompt.text} onPress={() => handlePromptSelection(prompt)} style={styles.button} />
                  )}
                </View>
          ...
          {!showPrompts && selectedPrompt && selectedPrompt.text === 'Rate your driver' && (
            <View style={{ flexDirection: 'column', alignItems: 'center', padding: 16 }}>
              ...
              <Button color='green' title="Ask more" onPress={handleAskMore} style={styles.button} />
            </View>
          )}
          ...
          {!showPrompts && selectedPrompt && selectedPrompt.text !== 'Rate your driver' && (
            <View style={{ flexDirection: 'row', alignItems: 'center', padding: 16 }}>
              ...
              />
              <Button color='green' title="Send" onPress={handleSend} style={styles.button} />
              <Button color='green' title="Go back" onPress={handleGoBack} style={styles.button} />
            </View>
          )}
        </View>
      </ScrollView>
